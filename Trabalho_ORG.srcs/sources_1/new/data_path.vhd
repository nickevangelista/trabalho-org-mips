...
component ula 
    Port (
         ...
         ula_op  : in  std_logic_vector(1 downto 0);                --add (00), sub(10), and(01)
...
component Mux_Data 
    Port(
        ula_in       : in  std_logic_vector(31 downto 0); -- dado vindo da ULA
...
--        Registradores intermediarios
    ...
    signal ULA_OUT  :std_logic_vector(31 downto 0); -- registrador ULA_OUT para salvar o resultado da ULA
 ...
 --       Fios da parte operativa
    ...
    signal PC_plus_4    :std_logic_vector(9 downto 0);  -- sinal que sai do somador do pc+1 para o mux_pc (pc + 1)
    signal zero_BEQ     :std_logic;
    signal MEM_ADR      :std_logic_vector(9 downto 0);  -- sinal que sai do mux_mem e entra no endereco de mem
    ...
    signal RegB_OUT     :std_logic_vector(31 downto 0); -- sinal que sai do Reg B e entra na ULA e no Mux_mem (store)
 ...
 ------------------- 
 -- Campos comuns para as instruções
...
-- Endereço para JUMP
    ...
    signal write_reg    : std_logic_vector(3 downto 0); -- guarda qual registrador vai receber a escrita
    
----------------------------------------------
    signal Next_Pc_target       : std_Logic_vector(9 downto 0); -- salva o endereço de um jmp, beq, store ou load
...
process(clk, reset)
    begin
        ...
        elsif rising_edge(clk) then  -- Atuliza os registradores secundários
